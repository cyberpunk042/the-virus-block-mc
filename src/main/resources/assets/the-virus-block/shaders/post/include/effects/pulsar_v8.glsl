...
#ifndef PULSAR_V8_GLSL
#define PULSAR_V8_GLSL
...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 5: Electric Plasma Functions
// ═══════════════════════════════════════════════════════════════════════════

#include "electric_plasma.glsl"  // animatedElectricCoronaV8 and helpers
#include "../utils/core_patterns.glsl"  // V8 core type switching (Default/Electric)
...
#define PulsarV8Result EffectResult
...
PulsarV8Result renderPulsarV8(
    ...
    float sceneDistance,      // Distance to scene geometry (from depth buffer)
    bool isSky                // Is this pixel sky?
) {
    PulsarV8Result result;
    result.didHit = false;
    ...
    if (effectVisibility > 0.01 && detail >= 1) {
        // ═══════════════════════════════════════════════════════════════════
        // CORE TYPE SWITCHING
        // V8CoreType: 0=Default (V7 noise sphere), 1=Electric (FBM tendrils)
        //
        // PARAMETER REUSE RATIONALE:
        // - Electric mode uses NoiseOctaves for layer count (semantic: FBM iterations)
        // - Electric mode uses V2CoreSpread for pattern scale (repurpose: core spread → pattern scale)
        // - Electric mode uses SpeedHigh for animation (repurpose: fast detail → electric flow)
        // - Electric mode uses V8PlasmaIntensity for brightness (repurpose: plasma → core glow)
        // ═══════════════════════════════════════════════════════════════════
        
        int coreType = int(V8CoreType);
        
        if (coreType == 1) {
            // ─── ELECTRIC CORE MODE ───
            // Uses FBM-based electric tendrils with optional flash and fill
            // Calculate distance from orb SURFACE (not center)
            float distanceFromSurface = max(0.0, distCamToSun - radius);
            
            vec3 electricCol = electricCorePattern(
                surface,                    // Position on sphere surface
                fragTime,                   // Animation time
                int(NoiseOctaves),          // Layer count (reused from Noise)
                V2CoreSpread,               // Pattern scale (from Core Spread control)
                speedHi,                    // Animation speed (reused from SpeedHigh)
                V8PlasmaIntensity,          // Brightness (reused from V8Plasma)
                V8ElectricFlash,            // Flash toggle (0=off, 1=on scene flash)
                V8ElectricFillIntensity,    // Fill visibility (0=minimal, 1=rich)
                vec3(SecondaryR, SecondaryG, SecondaryB),  // Fill color (Accent1)
                V8ElectricFillDarken,       // Fill darken (0=full color, 1=black)
                V8ElectricLineWidth,        // Line thickness (-1000 to +1000)
                bodyColor,                  // Body color for tinting
                result.didHit,              // Whether ray hit sphere (controls flash)
                distanceFromSurface,        // Distance from orb surface (for flash fade)
                DistortionRadius            // Fade range from Proximity Darken config
            );
            
            // Electric pattern already includes body color blending via fill
            col.rgb = electricCol;
            
            // Apply occlusion bleed
            col.rgb *= effectVisibility;
        } else {
            // ─── DEFAULT CORE MODE (V7) ───
            // Unity seed vectors:
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // SUBTRACTIVE RING - DISABLED FOR V8
    // (V7 legacy feature that doesn't fit V8 aesthetic)
    // ═══════════════════════════════════════════════════════════════════════
    
    // ═══════════════════════════════════════════════════════════════════════
    // RAY CORONA (V8: Full Control - V7 params + V8 params)
    // ═══════════════════════════════════════════════════════════════════════
    
    vec3 seedVec = vec3(83.23, 34.34, 67.453) * seed;
    
    float s3 = animatedElectricCoronaV8(
        // ─── Core inputs (9) ───
        pr, viewRotation,
        radius, zoom,
        sphere, sqRadius,
        ...
        // ─── Ground transposition (6) ───
        sceneDistance,
        isSky,
        rayOrigin,
        ray,
        forward,        // Camera forward for Z-depth to ray distance conversion
        sphereCenter,
        
        // ─── V7 PARAMS (9) - from function signature, all working ───
        speedRing,      // Ring noise animation
        speedRay,       // Ray noise animation
        rayString,      // Ring width
        rayReach,       // Ring extent
        rays,           // Ray intensity exponent
        rayRing,        // Ring noise exponent
        rayGlow,        // Ray-modulated glow
        glow,           // Base glow
        rayFade,        // Edge fade
        
        // ─── V8 PLASMA (4) - from UBO ───
        V8PlasmaScale,
        V8PlasmaSpeed,
        V8PlasmaTurbulence,
        V8PlasmaIntensity,
        ...
        // ─── V8 CORONA (4) - from UBO ───
        V8CoronaExtent,
        V8CoronaFadeStart,
        V8CoronaFadePower,
        V8CoronaIntensity,
        
        // ─── Proximity darkness range ───
        max(10.0, DistortionRadius)  // darkenRange
    );
